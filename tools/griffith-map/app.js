/**
 * Electorate Map Application
 * Interactive map with MapLibre and OpenFreeMap
 */

// Electorate coordinates
const ELECTORATE_CENTRE = {
    lng: 153.0281,
    lat: -27.4989,
    zoom: 12
};
...
const ELECTORATE_GEOJSON_PATH = 'griffith.geojson';
...
const METRIC_CONFIG = {
    fp_alp: {
        ...
        min: 0.25, max: 0.36,
        colorStops: [0.25, '#f59d9d', 0.36, '#a80a1c'],
        legendGradient: 'linear-gradient(to right, #f59d9d, #a80a1c)',
        legendMin: '25%', legendMax: '36%'
    },
    fp_lnp: {
        label: 'LIB First Preference',
        min: 0.21, max: 0.33,
        colorStops: [0.21, '#ffffff', 0.33, '#003087'],
        legendGradient: 'linear-gradient(to right, #ffffff, #003087)',
        legendMin: '21%', legendMax: '33%'
    },
    tcp_alp: {
        label: 'ALP Two-Candidate Preferred',
        min: 0, max: 1,
        colorStops: [.45, '#44ab08', 0.5, '#ffffff', .65, '#e3142d'],
        legendGradient: 'linear-gradient(to right, #44ab08, #ffffff, #e3142d)',
...
function updateMapContainerOffset() {
    ...
    document.documentElement.style.setProperty('--electorate-header-offset', `${headerHeight}px`);
...
function initMap() {
    map = new maplibregl.Map({
        ...
        center: [ELECTORATE_CENTRE.lng, ELECTORATE_CENTRE.lat],
        zoom: ELECTORATE_CENTRE.zoom,
    ...
    map.on('load', async () => {
        await addElectorateLayer();
        await addBoothsLayer();
        await addElectionLayer({ year: '2025', csvPath: SA1_CSV_2025 });
        await addElectionLayer({ year: '2022', csvPath: SA1_CSV_2022 });
    });
}

/**
 * Add Griffith boundary layer to the map
 */
async function addElectorateLayer() {
    try {
        const response = await fetch(ELECTORATE_GEOJSON_PATH);
        if (!response.ok) {
            throw new Error(`Failed to load Griffith boundary: ${response.status} ${response.statusText}`);
        }

        const electorateGeoJSON = await response.json();

        map.addSource('electorate-boundary', {
            type: 'geojson',
            data: electorateGeoJSON
        });

        map.addLayer({
            id: 'electorate-fill',
            type: 'fill',
            source: 'electorate-boundary',
            paint: {
                'fill-color': '#22c55e',
                'fill-opacity': 0.15
            }
        ...
        console.error('Error loading Griffith boundary:', error);
    }
}
    ...
    try {
        ...
        geojson.features.forEach((feature) => {
            ...
            if (row) {
                ...
                feature.properties.tcp_alp = parseFloat(row.tcp_alp);
            }
        });
